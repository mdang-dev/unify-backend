pipeline {
    ...
    stages {
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${latest}' 'ec2-13-229-58-11.ap-southeast-1.compute.amazonaws.com:8080/job/unify-backend-cd/buildWithParameters?token=unify-gitops-token'"
                }
            }
        }
    }
